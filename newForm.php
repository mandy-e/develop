...
require_once('../bin/myDatabase.php');
	...
	$queryVoicing = 'SELECT DISTINCT fldVoicing ';
	$queryVoicing .= ' FROM tblSheetMusic ';
	$queryVoicing .= ' ORDER BY fldVoicing ';
	print "<p>queryVoicing = " . $queryVoicing;
	
	$queryPrimary_Language = 'SELECT DISTINCT fldPrimaryLanguage ';
	$queryPrimary_Language .= ' FROM tblSheetMusic ';
	$queryPrimary_Language .= ' ORDER BY fldPrimaryLanguage ';
	print "<p>queryPrimary_Language = " . $queryPrimary_Language;
	
	$querySecondary_Language = 'SELECT DISTINCT fldSecondaryLanguage ';
	$querySecondary_Language .= ' FROM tblSheetMusic ';
	$querySecondary_Language .= ' ORDER BY fldSecondaryLanguage ';
	print "<p>querySecondary_Language = " . $querySecondary_Language;
	
	$queryChristmas = 'SELECT DISTINCT fldChristmasStatus ';
	$queryChristmas .= ' FROM tblSheetMusic ';
	$queryChristmas .= ' ORDER BY fldChristmasStatus ';
	print "<p>queryChristmas = " . $queryChristmas;
	
	$queryAcappella = 'SELECT DISTINCT fldAcappellaStatus ';
	$queryAcappella .= ' FROM tblAccompanyment ';
	$queryAcappella .= ' ORDER BY fldAcappellaStatus ';
	print "<p>queryAcappella = " . $queryAcappella;
	
	$queryPiano = 'SELECT DISTINCT fldPianoStatus ';
	$queryPiano .= ' FROM tblAccompanyment ';
	$queryPiano .= ' ORDER BY fldPianoStatus ';
	print "<p>queryPiano = " . $queryPiano;
...
//print_r ($publisher);
//die();
$voicing = $thisDatabase->select($queryVoicing);
$primary_language = $thisDatabase->select($queryPrimary_Language);
$secondary_language = $thisDatabase->select($querySecondary_Language);

$christmas = $thisDatabase->select($queryChristmas);
$acappella = $thisDatabase->select($queryAcappella);
$piano = $thisDatabase->select($queryPiano);
            ...
            <fieldset class="wrapper">
                ...
                <fieldset class="wrapperTwo">
                    ...
                    <!--<fieldset class="contact">
                        ...
                        <?php 	
                        //start of source listbox
						// or you can print it out
						print '</select></label>';	
                        print '<label for="lstSource">Text Source ';
						print '<select id="lstSource" ';
						print '        name="lstSource"';
						print '        tabindex="150" >';
						...
						foreach ($source as $row) {
							...
							print '<option ';
							if ($source == $row["fldSource"])
								print " selected ";

							print 'value="' . $row["fldSource"] . '">' . $row["fldSource"];

							print '</option>';
						}

						print '</select></label>';
						//end of source listbox output
						...
						$output[] = '<legend>Which vocal range would you like? (check all that apply):</legend>';
						...
						foreach ($voicing as $row) {
							...
							//$output[] = 'value="' . $row[$thisDatabase->lastInsertId("pmkSheetMusicId")] . '">' . $row["fldVoicing"];
						...
						$output = array();
						$output[] = '<fieldset class="checkbox">';
						$output[] = '<legend>Which primary language would you like? (check all that apply):</legend>';
						...
						$output = array();
						$output[] = '<fieldset class="checkbox">';
						$output[] = '<legend>Which optional language would you like? (check all that apply):</legend>';
						...
						foreach ($christmas as $row) {
							...
							if ($ichristmas == $row["pmkSheetMusicId"])
								$output[] =  " checked ";
						...
						foreach ($acappella as $row) {
							...
							if ($iacappella == $row["pmkAccompanymentId"])
								$output[] =  " checked ";
						...
						foreach ($piano as $row) {
							...
							if ($ipiano == $row["pmkAccompanymentId"])
								$output[] =  " checked ";
